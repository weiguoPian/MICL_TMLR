...
# from lavis.models.blip2 import Blip2Base, disabled_train, LayerNorm
...
class Blip2VicunaXInstructOnlyProjwithTemporal(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "vicuna7b": "lavis/configs/models/blip2/blip2_xinstruct_vicuna7b.yaml",
        "vicuna13b": "lavis/configs/models/blip2/blip2_xinstruct_vicuna13b.yaml",
    }
    ...
    def __init__(
        ...
        from transformers import LlamaTokenizer, AutoTokenizer, AutoModelForCausalLM, AutoConfig, PretrainedConfig, LlamaConfig
        from lavis.models.blip2_models.modeling_llama import LlamaForCausalLM
        # from lavis.models.modeling_llama import LlamaForCausalLM
        ...
        self.llm_tokenizer = AutoTokenizer.from_pretrained(llm_model, truncation_side="left")
        if 'Llama' in llm_model:
            print('Llama in llm_model')
            self.llm_name = 'Llama'
            # self.llm_tokenizer = AutoTokenizer.from_pretrained(llm_model, truncation_side="left")
            ...
            print('Vicuna in llm_model')
            self.llm_name = 'Vicuna'
        ...
        if self.lora:
            if 'Llama' in llm_model:
                ...
                try:
                    self.llm_model = AutoModelForCausalLM.from_pretrained(
                        llm_model, load_in_8bit=True, torch_dtype=torch.float16)
                except:
                    llm_config_dict = PretrainedConfig.from_pretrained(llm_model).to_dict()
                    llm_config_dict["rope_scaling"] = {"factor": 8.0, "type": "dynamic"}
                    llm_config = LlamaConfig.from_dict(llm_config_dict)
                    self.llm_model = AutoModelForCausalLM.from_pretrained(
                        llm_model, config=llm_config, load_in_8bit=True, torch_dtype=torch.float16
                    )
            else:
                ...
                # )
                print('self.llm_name: {}'.format(self.llm_name))
                self.llm_model = AutoModelForCausalLM.from_pretrained(
                    llm_model, load_in_8bit=True, torch_dtype=torch.float16)
            ...
            self.llm_device = self.llm_model.get_input_embeddings().weight.device
            ...
            if 'Llama' in llm_model:
                ...
                try:
                    ...
                    self.llm_model = AutoModelForCausalLM.from_pretrained(
                        llm_model, config=llm_config, torch_dtype=torch.float16)
            else:
                # print('Vicuna in llm_model')
                print('self.llm_name: {}'.format(self.llm_name))
                self.llm_model = LlamaForCausalLM.from_pretrained(
        ...
        for modality in self.modalities:
            ...
            if self.projection_only or locals()[f"projection_only_{modality}"]:
                ...
                print("modality_encoder.num_features: {}".format(getattr(self, f"{modality}_encoder").num_features))
                
                # if modality == 'video' or modality == 'audio':
                #     modality_projection = self.init_modality_projection(
                #         modality_num_features*4, 
                #         self.llm_hidden_size)
                # else:
                #     modality_projection = self.init_modality_projection(
                #         modality_num_features, 
                #         self.llm_hidden_size)
                # modality_conv_pooling = self.init_modality_conv_pooling(
                #     modality_num_features, 
                #     self.llm_hidden_size)

                if modality == 'audio' or modality == 'video':
                    modality_conv_pooling = self.init_modality_conv_pooling(
                        modality_num_features, 
                        modality_num_features)
                    setattr(self, f"{modality}_conv_pooling", modality_conv_pooling)
                ...
                # if modality == 'video' or modality == 'audio':
                #     modality_temporal_projection = self.init_modality_temporal_projection(
                #         input_size=4, output_size=1
                #     )
                #     setattr(self, f"{modality}_temporal_projection", modality_temporal_projection)
        ...
        if self.use_cues:
            # logging.info(f"Modality to cue {Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE}")
            self.tokenized_cue = {}
            self.emb_cue = {}
            self.att_cue = {}
            for modality in self.modalities:
                if self.clean_tokenization:
                    Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[modality] = Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[modality].lstrip()
                self.tokenized_cue[modality] = self.llm_tokenizer(Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[modality], return_tensors="pt")
                ...
                # except:
                self.emb_cue[modality] = self.llm_model.get_input_embeddings()(self.tokenized_cue[modality].input_ids.to(self.llm_device))
                self.att_cue[modality] = self.tokenized_cue[modality].attention_mask.to(self.llm_device)
    ...
    def forward(self, samples, with_text_out=True):
        ...
        for modality in curr_modalities:
            ...
            if modality == "video" and self.video_enc_name in Blip2VicunaXInstructOnlyProjwithTemporal.SEQUENCIAL_ENCODERS:
                embeds[modality] = []
                inputs_llm[modality] = []
                # data_atts[modality] = []
                for j in range(data.size(2)):
                    ...
                    with self.maybe_autocast():
                        ...
                        # inputs_llm[modality].append(getattr(self, f"{modality}_projection")(embeds[modality][j]))
                        inputs_llm[modality].append(embeds[modality][j])
                
                hw = 16
                inputs_llm[modality] = torch.stack(inputs_llm[modality], dim=1)[:, :, 1:, :]
                BS = inputs_llm[modality].shape[0]
                T = inputs_llm[modality].shape[1]
                encoder_feat_size = inputs_llm[modality].shape[-1]
                inputs_llm[modality] = inputs_llm[modality].view(BS, T, hw, hw, encoder_feat_size)
                inputs_llm[modality] = inputs_llm[modality].view(-1, hw, hw, encoder_feat_size)
                inputs_llm[modality] = inputs_llm[modality].permute(0, 3, 1, 2)
                inputs_llm[modality] = getattr(self, f"{modality}_conv_pooling")(inputs_llm[modality])
                new_hw = inputs_llm[modality].shape[-1]
                # (BS, T, new_hw, new_hw, encoder_feat_size)
                inputs_llm[modality] = inputs_llm[modality].view(BS, T, encoder_feat_size, new_hw, new_hw).permute(0, 1, 3, 4, 2)
                # inputs_llm[modality] = inputs_llm[modality].view(BS, T, new_hw*new_hw, encoder_feat_size).view(BS, T*new_hw*new_hw, encoder_feat_size)
                inputs_llm[modality] = inputs_llm[modality].view(BS, -1, encoder_feat_size)


                inputs_llm[modality] = getattr(self, f"{modality}_projection")(inputs_llm[modality])
                # {modality}_projection

                # inputs_llm[modality] = torch.cat(inputs_llm[modality], dim=-1)
                # inputs_llm[modality] = getattr(self, f"{modality}_projection")(inputs_llm[modality])
                # inputs_llm[modality] = torch.cat(inputs_llm[modality], dim=1)
                # inputs_llm[modality] = getattr(self, f"{modality}_temporal_projection")(torch.stack(inputs_llm[modality], dim=-1)).squeeze(-1)
                # inputs_llm[modality] = torch.stack(inputs_llm[modality], dim=1).mean(dim=1)
                atts_llm[modality] = torch.ones(inputs_llm[modality].size()[:-1], dtype=torch.long).to(self.device)

            elif modality == 'audio' and self.audio_enc_name in Blip2VicunaXInstructOnlyProjwithTemporal.SEQUENCIAL_ENCODERS:
                embeds[modality] = []
                inputs_llm[modality] = []
                # data_atts[modality] = []
                ...
                for j in range(data.size(1)):
                    this_frame = data[:,j,:,:]
                    with self.maybe_autocast():
                        # print('this_frame.shape: {}'.format(this_frame.shape))
                        embeds[modality].append(ln(encoder(this_frame)))
                        # inputs_llm[modality].append(getattr(self, f"{modality}_projection")(embeds[modality][j]))
                        inputs_llm[modality].append(embeds[modality][j])
                
                hw = 16
                inputs_llm[modality] = torch.stack(inputs_llm[modality], dim=1)
                BS = inputs_llm[modality].shape[0]
                T = inputs_llm[modality].shape[1]
                encoder_feat_size = inputs_llm[modality].shape[-1]
                inputs_llm[modality] = inputs_llm[modality].view(BS, T, hw, hw, encoder_feat_size)
                ...
                inputs_llm[modality] = getattr(self, f"{modality}_projection")(embeds[modality])

                # hw = 16
                # BS = embeds[modality].shape[0]
                # encoder_feat_size = embeds[modality].shape[-1]
                # embeds[modality] = embeds[modality][:, 1:, :]
                # embeds[modality] = embeds[modality].view(BS, hw, hw, encoder_feat_size).permute(0, 3, 1, 2)
                # # (BS, encoder_feat_size, new_hw, new_hw)
                # embeds[modality] = getattr(self, f"{modality}_conv_pooling")(embeds[modality])
                # new_hw = embeds[modality].shape[-1]
                # # (BS, new_hw*new_hw, encoder_feat_size)
                # embeds[modality] = embeds[modality].permute(0, 2, 3, 1).view(BS, -1, encoder_feat_size)
        ...
        if with_text_out:
            ...
            if self.llm_name == 'Qwen':
                text_output_tokens = self.llm_tokenizer(
                    [self.llm_tokenizer.pad_token + t + self.llm_tokenizer.eos_token for t in samples['text_output']],
                    return_tensors="pt",
                    padding="longest",
                    truncation=True,
            ...
            input_part_targets_len = []
            for i in range(text_input_tokens.input_ids.size(0)):
                this_input_ones = text_input_tokens.attention_mask[i].sum()
                input_part_targets_len.append(this_input_ones)
        ...
        if with_text_out:
            ...
            loss = outputs.loss
            logits = outputs.logits[..., :-1, :].contiguous().view(-1, self.llm_model.config.vocab_size)
    ...
    def get_query_outputs(
        ...
        if self.qformer_text_input:
            ...
            for modality in curr_modalities:
                ...
                if modality in Blip2VicunaXInstructOnlyProjwithTemporal.SEQUENCIAL_MODALITIES and getattr(self, f'{modality}_enc_name') in Blip2VicunaXInstructOnlyProjwithTemporal.SEQUENCIAL_ENCODERS:
    ...
    def generate(
        ...
        #     print("Model modalities do not match sample modalities.")
        #     return
        # print('curr_modalities: {}'.format(curr_modalities))
        # get batch size
        bs = None
        if len(curr_modalities) == 0:
            # bs = 1
        ...
        # if max_length > inputs_embeds.shape[1]:
        #     out_max_length = max_length
        # else:
        #     out_max_length = inputs_embeds.shape[1] + 1
        # print('######################################')
        ...
        ######################################
        # new_version max_new_tokens
        # old_version max_length
        ######################################

        outputs[outputs == self.llm_tokenizer.unk_token_id] = self.llm_tokenizer.eos_token_id

        # if 'Llama' in self.llm_name:
        #     # unk_id = self.llm_tokenizer.convert_tokens_to_ids('<unk>')
        #     # eos_id = self.llm_tokenizer.convert_tokens_to_ids('</s>')
        #     outputs[outputs == self.llm_tokenizer.unk_token_id] = self.llm_tokenizer.eos_token_id
        #     # outputs[outputs == self.llm_tokenizer.pad_token_id] = self.llm_tokenizer.eos_token_id
        # else:
        #     # print('outputs1: {}'.format(outputs))
        #     outputs[outputs == 0] = 2 # convert output id 0 to 2 (eos_token_id)
        
        # print('outputs2: {}'.format(outputs))
        output_text = self.llm_tokenizer.batch_decode(outputs, skip_special_tokens=True)
        # print('output_text1: {}'.format(output_text))
        output_text = [o.strip() for o in output_text]
        # print('output_text2: {}'.format(output_text))
        ...
        **kwargs
        ...
        if 'discrn' in samples and self.use_caption: ## discriminatory reasoning
            ...
            if self.enumerate_inputs:
                prompt = [f'{self.prefix}(a){Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][0]] if self.use_cues else " "}{samples["baseline_captions"][i][0]} (b){Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][1]] if self.use_cues else " "}{samples["baseline_captions"][i][1]} {text_input[i]}' for i in range(bs)]
            else:
                prompt = [f'{self.prefix}{Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][0]]}{samples["baseline_captions"][i][0] if self.use_cues else " "}{Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][1]] if self.use_cues else " "}{samples["baseline_captions"][i][1]} {text_input[i]}' for i in range(bs)]
            llm_tokens = self.llm_tokenizer(
                prompt,
                padding="longest",
                return_tensors="pt"
        ...
        output_text, outputs = self.generate(
            samples,
            num_beams=num_beams,
            max_length=max_len,
            min_length=min_len,
    ...
    def _predict_class(
        ...
        if 'discrn' in samples and self.use_caption: ## discriminatory reasoning
            ...
            if self.enumerate_inputs:
                prompt = [f'{self.prefix}(a){Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][0]] if self.use_cues else " "}{samples["baseline_captions"][i][0]} (b){Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][1]] if self.use_cues else " "}{samples["baseline_captions"][i][1]} {prompt[i]}' for i in range(bs)]
            else:
                prompt = [f'{self.prefix}{Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][0]]}{samples["baseline_captions"][i][0] if self.use_cues else " "}{Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[samples["modalities"][i][1]] if self.use_cues else " "}{samples["baseline_captions"][i][1]} {prompt[i]}' for i in range(bs)]
            text_input_tokens = self.llm_tokenizer(
                prompt,
                padding="longest",
                return_tensors="pt"
            ...
            if self.qformer_text_input:
                if self.special_qformer_input_prompt or special_qformer_input_prompt:
                    ...
                    # qformer_prompt = [f'Describe the {Blip2VicunaXInstructOnlyProjwithTemporal.MODALITY_TO_CUE[modality].replace(":", "").strip() if modality != "pc" else "3d model"}.' for _ in samples["text_input"]]
                    text_Qformer = self.tokenizer(
                        qformer_prompt,
                        padding='longest',
                        truncation=True,
    ...
    @classmethod
    def init_modality_projection(self, input_size, output_size):
    ...
    @classmethod
    def init_modality_conv_pooling(self, in_channels, out_channels):
        # proj = nn.Linear(input_size, output_size)
        print('input_size: {}'.format(in_channels))
        print('output_size: {}'.format(out_channels))
    ...
    # @classmethod
    # def init_modality_temporal_projection(cls, input_size=4, output_size=1):
    #     # proj = nn.Linear(input_size, output_size)
    #     print('input_size: {}'.format(input_size))
    #     print('output_size: {}'.format(output_size))
    ...
    def load_checkpoint(self, ckpt_path, encoder_ln_proj_path=None, strict=False):
        unexpected_keys = []
        missing_keys = []
        if ckpt_path is not None:
            ...
            state_dict = {}
        if encoder_ln_proj_path is not None:
            encoder_ln_proj_dict = torch.load(encoder_ln_proj_path, map_location="cpu")
        else:
            encoder_ln_proj_dict = None
        print('encoder_ln_proj_dict.keys: {}'.format(encoder_ln_proj_dict.keys()))
        
        for modality in self.modalities:
            # if self.projection_only or getattr(self, f"projection_only_{modality}") :
            # logging.info(f"Loaded {modality} projection")
            if encoder_ln_proj_dict is None:
                if modality == 'audio' or modality == 'video':
                    modality_conv_pooling_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in state_dict.items() if f"{modality}_conv_pooling" == k.split('.')[0]}
                modality_projection_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in state_dict.items() if f"{modality}_projection" == k.split('.')[0]}
                # modality_temporal_projection_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in state_dict.items() if f"{modality}_temporal_projection" == k.split('.')[0]}
            else:
                if modality == 'audio' or modality == 'video':
                    modality_conv_pooling_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in encoder_ln_proj_dict.items() if f"{modality}_conv_pooling" == k.split('.')[0]}
                modality_projection_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in encoder_ln_proj_dict.items() if f"{modality}_projection" == k.split('.')[0]}
                # modality_temporal_projection_state_dict = {'.'.join(k.split('.')[1:]):v for k,v in encoder_ln_proj_dict.items() if f"{modality}_temporal_projection" == k.split('.')[0]}
            
            if modality == 'audio' or modality == 'video':
                msg = getattr(self, f"{modality}_conv_pooling").load_state_dict(modality_conv_pooling_state_dict, strict=strict)
                # print("modality_conv_pooling_state_dict.keys: {}".format(list(modality_conv_pooling_state_dict.keys())))
                # print("modality_projection_state_dict.keys: {}".format(list(modality_projection_state_dict.keys())))
                # print("{}_conv_pooling msg: {}".format(modality, msg))
                # print([k for k, v in getattr(self, f"{modality}_conv_pooling").named_parameters()])
                # print([k for k, v in getattr(self, f"{modality}_projection").named_parameters()])
                # exit(0)
                missing_keys.extend(msg.missing_keys)
                unexpected_keys.extend(msg.unexpected_keys)

            msg = getattr(self, f"{modality}_projection").load_state_dict(modality_projection_state_dict, strict=strict)
            missing_keys.extend(msg.missing_keys)
            unexpected_keys.extend(msg.unexpected_keys)
            
            # if modality == 'video' or modality == 'audio':
            #     if len(modality_temporal_projection_state_dict) > 0:
            #         msg = getattr(self, f"{modality}_temporal_projection").load_state_dict(modality_temporal_projection_state_dict, strict=strict)
            #         missing_keys.extend(msg.missing_keys)
            #         unexpected_keys.extend(msg.unexpected_keys)
            ...
            if getattr(self,f"load_ln_type_{modality}") == "vision":
                if encoder_ln_proj_dict is None:
                    ...
                    modality_ln_dict = {'.'.join(k.split('.')[1:]):v for k,v in encoder_ln_proj_dict.items() if f"ln_vision" in k.split('.')[0]}
            else:
                print('encoder_ln_proj_dict is None: {}'.format(encoder_ln_proj_dict is None))
                if encoder_ln_proj_dict is None:
                    modality_ln_dict = {'.'.join(k.split('.')[1:]):v for k,v in state_dict.items() if f"{modality}_ln" in k.split('.')[0]}
                    # modality_ln_dict = {'.'.join(k.split('.')[1:]):v for k,v in modality_encoder_projection_dict.items() if f"{modality}_ln" in k.split('.')[0]}
                else:
                    modality_ln_dict = {'.'.join(k.split('.')[1:]):v for k,v in encoder_ln_proj_dict.items() if f"{modality}_ln" in k.split('.')[0]}
            print('modality_ln_dict length: {}'.format(len(modality_ln_dict)))
            msg = getattr(self, f"{modality}_ln").load_state_dict(modality_ln_dict, strict=strict)
            print('{}_ln missing keys: {}'.format(modality, msg.missing_keys))
            missing_keys.extend(msg.missing_keys)
            unexpected_keys.extend(msg.unexpected_keys)
        ...
        if not self.lora or len(lora_state_dict) == 0:
            ...
            # loaded_lora_keys = [key for key in state_dict if key not in msg.missing_keys]
            # logging.info("Loader lora keys are : {}".format(loaded_lora_keys))
            missing_keys.extend(["llm_model."+k for k in msg.missing_keys])
        # ===========================================================
        # logging.info("Unloaded keys are : {}".format(missing_keys))
        # ===========================================================
